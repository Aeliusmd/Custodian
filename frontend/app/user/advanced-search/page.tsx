...
import { downloadUserDocumentFile } from '@/app/user/documents/lib/documentFileDownload';
...
function applyClientFilters(docs: DocumentRecord[], filters: SearchFilters): DocumentRecord[] {
  if (filters.docType === 'All') return docs;
  return docs.filter((d) => d.visibility === filters.docType);
}
...
export default function AdvancedSearchPage() {
  ...
  const loadFilterData = useCallback(async () => {
    try {
      ...
      const catsRes = await fetch(`${API_BASE_URL}/protected/user/categories`, { credentials: 'include' });
  ...
  const handleSearch = async () => {
    ...
    setSearchError('');
    try {
      const response = await fetch(`${API_BASE_URL}/protected/user/advanced-search`, {
        method: 'POST',
        credentials: 'include',
      ...
      setResults(applyClientFilters(apiResults, filters));
      setSearched(true);
  ...
  const openViewer = async (doc: DocumentRecord) => {
    setActiveMenu(null);
    try {
      const response = await fetch(`${API_BASE_URL}/protected/user/documents/${encodeURIComponent(doc.id)}`, {
        method: 'GET',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
      });
  ...
  const handleDownload = async (doc: DocumentRecord) => {
    setActiveMenu(null);
    await downloadUserDocumentFile(API_BASE_URL, doc, (message, type) =>
  ...
  const highlightKeyword = filters.keyword || filters.docName;
      ...
      <div className="px-4 py-5 sm:p-6 min-h-full font-inter">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
          <div className="min-w-0">
            ...
            <p className="text-sm text-gray-400 mt-0.5">Search your documents using metadata fields and attributes</p>
          </div>
          {searched && (
            <button
              type="button"
          ...
          <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-6 mb-6">
            ...
            <div className="mb-4">
              <label className="block text-xs font-semibold text-gray-400 uppercase tracking-wide mb-1.5">
                Keyword Search
                <span className="ml-2 normal-case font-normal text-gray-300">(name, category, metadata)</span>
              </label>
              <div className="relative">
                <i className="ri-search-line absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 text-base" aria-hidden />
                <input
