...
export default function UserAdvancedSearchPage() {
  const [categories, setCategories] = useState<Category[]>([]);
  const [loadingFilters, setLoadingFilters] = useState(true);
  const [filters, setFilters] = useState<SearchFilters>({
    docName: '', category: '', dateFrom: '', dateTo: '', keyword: '', metadata: {},
  });
  ...
  const loadFilterData = useCallback(async () => {
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/protected/user/categories`, { credentials: 'include' });
  ...
  useEffect(() => { void loadFilterData(); }, [loadFilterData]);
  ...
  const handleReset = () => {
    setFilters({ docName: '', category: '', dateFrom: '', dateTo: '', keyword: '', metadata: {} });
    setResults(null);
    setSearched(false);
    setSearchError('');
  ...
  const handleDownload = async (doc: DocumentRecord) => {
    setActiveMenu(null);
    try {
      const url = `${API_BASE_URL}/protected/user/documents/${encodeURIComponent(doc.id)}/file`;
      ...
      <div className="px-4 py-5 sm:p-6 min-h-full font-inter">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
          <div className="min-w-0">
            ...
            <p className="text-sm text-gray-400 mt-0.5">Filter your documents by attributes and metadata</p>
          </div>
          {searched && (
            <button
              type="button"
              ...
              <div className="md:col-span-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] gap-2 sm:items-center">
                  <input
                    ...
                    className="border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-600 outline-none focus:border-[#0097B2] bg-white cursor-pointer transition-all"
                  />
            ...
            {results.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-200 py-12 sm:py-16 px-4 text-center">
                ...
                <button
                  onClick={handleReset}
                  type="button"
                  className="px-4 py-2.5 rounded-xl text-sm font-medium text-white cursor-pointer whitespace-nowrap"
                  style={{ background: TEAL }}
                >
                  Reset Filters
                </button>
                        ...
                        <tr key={doc.id} className="border-b border-gray-50 last:border-0 hover:bg-[#0097B2]/[0.03] transition-all">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <div className="text-sm font-medium text-[#1a2340] truncate max-w-[220px]">
                                  {highlightText(doc.name, filters.docName)}
                                </div>
                          ...
                          <td className="px-5 py-4">
                            <div className="relative" onClick={(e) => e.stopPropagation()}>
                              ...
                              {activeMenu === doc.id && (
                                <div className="absolute right-0 top-10 w-44 bg-white border border-gray-200 rounded-xl overflow-hidden z-40 py-1.5">
                                  <button
                                    type="button"
