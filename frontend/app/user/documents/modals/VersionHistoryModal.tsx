...
export default function VersionHistoryModal({ doc, onClose }: Props) {
  ...
  const loadVersions = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE_URL}/protected/user/documents/${encodeURIComponent(doc.id)}/versions`, {
        credentials: 'include',
      });
      if (!res.ok) {
  ...
  const handleRestore = async (versionId: string, versionName: string) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE_URL}/protected/user/documents/${encodeURIComponent(doc.id)}/versions/${encodeURIComponent(versionId)}/restore`,
