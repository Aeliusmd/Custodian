...
export default function AllDocumentsPage() {
  ...
  const loadDocuments = useCallback(async () => {
    try {
      ...
      const [activeRes, archivedRes] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/protected/user/documents?archived=1`, { credentials: 'include' }),
      ]);
      if (!activeRes.ok) {
  ...
  const handleArchiveSingle = async (doc: DocumentRecord) => {
    try {
      const res = await fetch(`${API_BASE_URL}/protected/user/documents/${doc.id}/archive`, {
        method: 'PATCH',
        credentials: 'include',
  ...
  const handleMetaSave = async (doc: DocumentRecord, metadata: Record<string, string>) => {
    try {
      const res = await fetch(`${API_BASE_URL}/protected/user/documents/${doc.id}/metadata`, {
        method: 'PATCH',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
