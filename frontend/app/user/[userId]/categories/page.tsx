...
const fieldTypeIcon: Record<string, string> = {
  Text: 'ri-text', Number: 'ri-hashtag', Date: 'ri-calendar-line', Dropdown: 'ri-arrow-down-s-line',
  NIC: 'ri-id-card-line', Author: 'ri-user-line', Email: 'ri-mail-line', Phone: 'ri-phone-line', URL: 'ri-link',
};
...
export default function CategoriesPage() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const res = await fetch(`${API_BASE_URL}/protected/user/${userId}/categories`, { credentials: 'include' });
      if (!res.ok) {
    ...
    <div className="px-4 py-5 sm:p-6 space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between flex-wrap">
        <div className="min-w-0">
          ...
          <p className="text-gray-400 text-sm mt-0.5">{loading ? 'Loading...' : `${categories.length} categories with custom metadata`}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4">
        <div className="relative max-w-sm">
          <input type="text" placeholder="Search categories..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#0097B2] transition-colors" />
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              ...
              <tbody className="divide-y divide-gray-100">
                {filtered.map((cat) => (
                  <tr key={cat.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-5 py-4"><p className="text-sm text-gray-500 max-w-[200px] truncate">{cat.description || '—'}</p></td>
                    <td className="px-5 py-4">
                        ...
                        <div className="flex gap-1 flex-wrap">
                          {cat.fields.slice(0, 3).map((f) => (
                            <span key={f.id} className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-500 flex items-center gap-0.5 whitespace-nowrap">
                              <i className={`${fieldTypeIcon[f.type] ?? 'ri-text'} text-[10px]`} />{f.name}
                            </span>
                          ))}
                          {cat.fields.length > 3 && <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 text-gray-400">+{cat.fields.length - 3}</span>}
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-4"><span className="text-sm font-semibold text-[#1a2340]">{cat.docCount.toLocaleString()}</span></td>
                    <td className="px-5 py-4 text-sm text-gray-500 whitespace-nowrap">{cat.createdDate}</td>
                    <td className="px-5 py-4">
                      <button onClick={() => setViewCat(cat)} title="View details" className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-[#0097B2] hover:bg-[#0097B2]/10 transition-all">
        ...
        {!loading && filtered.length === 0 && <div className="py-16 text-center text-gray-400 text-sm">No categories found.</div>}
      </div>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-lg mx-4 max-h-[85vh] overflow-hidden flex flex-col">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              ...
              <button onClick={() => setViewCat(null)} aria-label="Close" title="Close" className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 transition-all">
