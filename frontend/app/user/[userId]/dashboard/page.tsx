...
export default function UserDashboard() {
  ...
  const load = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const res = await fetch(`${API_BASE_URL}/protected/user/${userId}/dashboard`, {
        credentials: 'include',
      });
      if (!res.ok) {
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <Link
          href={`/user/${userId}/upload-documents`}
            ...
            <div className="flex items-end gap-3 h-40">
              {uploadActivity.map((d) => (
                <div key={d.day} className="flex-1 flex flex-col items-center gap-1.5">
                  ...
                  <div className="w-full rounded-t-md transition-all" style={{ height: `${Math.max(4, (d.count / maxBar) * 120)}px`, background: TEAL, opacity: 0.7 }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <Link href={`/user/${userId}/documents`} className="text-xs font-medium hover:underline whitespace-nowrap" style={{ color: TEAL }}>View all</Link>
          </div>
          {loading ? (
              ...
              {recentDocs.map((doc) => (
                <div key={doc.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors cursor-pointer">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${doc.status === 'Public' ? 'bg-[#16a34a]/10 text-[#16a34a]' : 'bg-gray-100 text-gray-500'}`}>{doc.status}</span>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="px-5 py-3 border-t border-gray-50">
            <Link href={`/user/${userId}/documents`} className="text-xs font-medium hover:underline" style={{ color: TEAL }}>
              View all documents
            </Link>
          </div>
        </div>
