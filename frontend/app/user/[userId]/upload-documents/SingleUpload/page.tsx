...
import styles from '../../../upload-documents/upload-documents.module.css';
...
export default function SingleUpload() {
  ...
  useEffect(() => {
    fetch(`${API_BASE_URL}/protected/user/${userId}/categories`, { credentials: 'include' })
      .then((r) => r.json())
      .then((data) => {
        const cats = (Array.isArray(data) ? data : []).map((c: { id: string; name: string; fields?: CategoryField[] }) => ({
          id: c.id,
          name: c.name,
          fields: c.fields ?? [],
        }));
    ...
    try {
      ...
      if (selectedCat) {
        for (const field of selectedCat.fields) {
          const value = metaValues[field.id]?.trim();
          if (value) metadataByName[field.name] = value;
        }
      }

      const res = await fetch(`${API_BASE_URL}/protected/user/${userId}/documents/single`, {
        method: 'POST',
        credentials: 'include',
      ...
      <div className={styles.uploadFrame}>
        <div className={styles.uploadHeaderWrap}>
          <div className={styles.uploadHeaderRow}>
            <button
              type="button"
              onClick={() => router.push(`/user/${userId}/upload-documents`)}
          ...
          <div className={styles.uploadPanelInner}>
                    ...
                    {categories.map((category) => {
                      ...
                      return (
                        <button
                          key={category.id}
                          type="button"
                          onClick={() => { setSelectedCatId(category.id); setErrors({}); }}
            ...
            {step === 1 && (
              <div className="space-y-4">
                ...
                >
                  ...
                  <input ref={fileRef} type="file" className="hidden" aria-label="Upload a single file" title="Upload a single file" onChange={handleFileInput} />
                </div>
                {file && (
                  <div className="flex items-center gap-3 p-4 bg-white border border-gray-200 rounded-xl">
                    ...
                    <button type="button" onClick={() => setFile(null)} className={`w-7 h-7 flex items-center justify-center rounded-lg transition-all ${styles.iconOnlyButton}`} aria-label={`Remove ${file.name}`}>
                ...
                <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
                  {selectedCat.fields.map((field) => (
                    <div key={field.id}>
                      ...
                      {field.type === 'Text' && (
                        <input id={field.id} type="text" value={metaValues[field.id] ?? ''} onChange={(e) => setMetaValues((v) => ({ ...v, [field.id]: e.target.value }))} placeholder={`Enter ${field.name}`} aria-label={field.name} className={`w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none transition-colors ${errors[field.id] ? 'border-red-300 bg-red-50' : 'border-gray-200 focus:border-[#0097B2]'}`} />
                      )}
                      {field.type === 'Number' && (
                        <input id={field.id} type="number" value={metaValues[field.id] ?? ''} onChange={(e) => setMetaValues((v) => ({ ...v, [field.id]: e.target.value }))} placeholder="0" aria-label={field.name} className={`w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none transition-colors ${errors[field.id] ? 'border-red-300 bg-red-50' : 'border-gray-200 focus:border-[#0097B2]'}`} />
                      )}
                      {field.type === 'Date' && (
                        <input id={field.id} type="date" value={metaValues[field.id] ?? ''} onChange={(e) => setMetaValues((v) => ({ ...v, [field.id]: e.target.value }))} aria-label={field.name} className={`w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none transition-colors ${errors[field.id] ? 'border-red-300 bg-red-50' : 'border-gray-200 focus:border-[#0097B2]'}`} />
                      )}
                      {field.type === 'Dropdown' && (
                        <select id={field.id} value={metaValues[field.id] ?? ''} onChange={(e) => setMetaValues((v) => ({ ...v, [field.id]: e.target.value }))} aria-label={field.name} className={`w-full px-3 py-2.5 border rounded-lg text-sm focus:outline-none transition-colors ${errors[field.id] ? 'border-red-300 bg-red-50' : 'border-gray-200 focus:border-[#0097B2]'}`}>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {(['Public', 'Private'] as const).map((option) => {
                    ...
                    return (
                      <button key={option} type="button" onClick={() => setPrivacy(option)} className={`p-5 rounded-xl border-2 text-left transition-all ${active ? 'border-[#0097B2] bg-[#0097B2]/5' : 'border-gray-200 hover:border-gray-300 bg-white'}`} aria-pressed={active}>
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          {active && <div className="ml-auto w-5 h-5 rounded-full flex items-center justify-center bg-[#0097B2]" aria-hidden="true"><i className="ri-check-line text-white text-xs" /></div>}
                        </div>
                        <div className="text-sm font-semibold text-[#1a2340]">{option}</div>
                        <div className="text-xs text-gray-400 mt-1">{option === 'Public' ? 'Visible to all users in the organization' : 'Only accessible to authorized users'}</div>
                      </button>
                    );
                  })}
                </div>
            ...
            {step === 4 && (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-400 mb-1"><strong className="text-[#1a2340]">{file?.name}</strong> has been successfully uploaded</p>
                <p className="text-xs text-gray-400">Category: {selectedCat?.name} · {privacy}</p>
                <div className="flex flex-col sm:flex-row gap-3 justify-center mt-8">
                  <button type="button" onClick={() => router.push(`/user/${userId}/documents`)} className="px-5 py-2.5 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors whitespace-nowrap">
                    View Documents
                  </button>
                  <button type="button" onClick={() => { setStep(0); setSelectedCatId(''); setFile(null); setMetaValues({}); setPrivacy('Private'); setErrors({}); setUploadError(''); }} className="px-5 py-2.5 rounded-lg text-white text-sm font-medium transition-colors whitespace-nowrap bg-[#0097B2] hover:bg-[#007d95]">
                    Upload Another
                  </button>
                </div>
              </div>
            ...
            {step < 4 && (
              <div className="flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-3 mt-8 pt-5 border-t border-gray-100">
                <button type="button" onClick={() => (step === 0 ? router.push(`/user/${userId}/upload-documents`) : setStep((s) => s - 1))} className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg border border-gray-200 text-sm font-medium text-gray-600 hover:bg-gray-50 transition-colors whitespace-nowrap">
                  <i className="ri-arrow-left-line" aria-hidden="true" />
                  {step === 0 ? 'Cancel' : 'Back'}
                </button>
                {step < 3 ? (
                  <button type="button" onClick={next} className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg text-white text-sm font-medium transition-colors whitespace-nowrap bg-[#0097B2] hover:bg-[#007d95]">
                    Continue <i className="ri-arrow-right-line" aria-hidden="true" />
                  </button>
                ) : (
                  <button type="button" onClick={handleFinalUpload} disabled={uploading} className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg text-white text-sm font-medium transition-colors whitespace-nowrap disabled:opacity-70 bg-[#0097B2] hover:bg-[#007d95]">
                    {uploading ? <><i className="ri-loader-4-line animate-spin" aria-hidden="true" />Uploading...</> : <><i className="ri-upload-cloud-2-line" aria-hidden="true" />Upload Document</>}
                  </button>
                )}
              </div>
            )}
