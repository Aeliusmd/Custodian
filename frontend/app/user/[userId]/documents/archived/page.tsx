...
export default function ArchivedDocumentsPage() {
  ...
  const loadArchived = useCallback(async () => {
    try {
      setLoading(true);
      setError('');
      const res = await fetch(`${API_BASE_URL}/protected/user/${userId}/documents?archived=1`, { credentials: 'include' });
      if (!res.ok) {
  ...
  const handleRestore = async (ids: string[]) => {
    try {
      await Promise.all(ids.map((id) =>
        fetch(`${API_BASE_URL}/protected/user/${userId}/documents/${id}/archive`, {
  ...
  const handlePermanentDelete = async (docs: DocumentRecord[]) => {
    ...
    try {
      await Promise.all(ids.map((id) =>
        fetch(`${API_BASE_URL}/protected/user/${userId}/documents/${id}`, { method: 'DELETE', credentials: 'include' })
      ));
      setArchivedDocs((prev) => prev.filter((d) => !ids.includes(d.id)));
      setSelected(new Set());
    ...
    <div className="p-6 min-h-full font-inter">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.push(`/user/${userId}/documents`)}
      ...
      <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200 mb-5">
        ...
        <div>
          ...
          <p className="text-xs text-amber-600 mt-0.5">You can restore them at any time or permanently delete them.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-4">
        <div className="flex items-center gap-2 border border-gray-200 rounded-lg px-3 py-2 max-w-sm">
          ...
          <input placeholder="Search archived documents..." value={search} onChange={(e) => setSearch(e.target.value)} className="flex-1 outline-none text-sm text-[#1a2340] bg-transparent" />
      ...
      {selected.size > 0 && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl mb-4 text-white" style={{ background: '#92400e' }}>
          ...
          <button onClick={() => void handleRestore(Array.from(selected))} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/20 hover:bg-white/30 text-sm font-medium transition-all cursor-pointer whitespace-nowrap">
            <i className="ri-inbox-unarchive-line" /> Restore Selected
          </button>
          <button onClick={() => setConfirmDelete(archivedDocs.filter((d) => selected.has(d.id)))} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-500/80 hover:bg-red-600 text-sm font-medium transition-all cursor-pointer whitespace-nowrap">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-sm text-gray-400">{search ? 'No documents match your search' : 'Documents you archive will appear here'}</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-100">
                  ...
                  {['Document Name', 'Category', 'Uploaded By', 'Upload Date', 'Actions'].map((h) => (
              ...
              <tbody>
                {filtered.map((doc) => (
                  <tr key={doc.id} className="border-b border-gray-50 last:border-0 hover:bg-amber-50/30 transition-all cursor-pointer" onClick={() => setViewerDoc(doc)}>
                    ...
                    <td className="px-4 py-3.5"><span className="text-xs font-medium px-2.5 py-1 rounded-full bg-gray-100 text-gray-600 whitespace-nowrap">{doc.category}</span></td>
                    <td className="px-4 py-3.5">
                      <div className="flex items-center gap-2">
                    ...
                    <td className="px-4 py-3.5" onClick={(e) => e.stopPropagation()}>
                      <div className="flex items-center gap-2">
                        <button onClick={() => void handleRestore([doc.id])} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold transition-all cursor-pointer whitespace-nowrap" style={{ color: TEAL, background: `${TEAL}12` }}>
                          <i className="ri-inbox-unarchive-line text-xs" /> Restore
                        </button>
                        <button onClick={() => setConfirmDelete([doc])} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-red-500 bg-red-50 hover:bg-red-100 transition-all cursor-pointer whitespace-nowrap">
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-sm mx-4 overflow-hidden">
            ...
            <div className="p-6 flex flex-col items-center gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500">This will permanently delete <strong>{confirmDelete.length} document{confirmDelete.length > 1 ? 's' : ''}</strong>. This cannot be undone.</p>
              </div>
              ...
              <div className="flex gap-3 w-full">
                <button onClick={() => setConfirmDelete(null)} className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm font-semibold text-gray-600 hover:bg-gray-50 transition-colors cursor-pointer whitespace-nowrap">Cancel</button>
                <button onClick={() => void handlePermanentDelete(confirmDelete)} className="flex-1 px-4 py-2.5 rounded-xl bg-red-500 text-white text-sm font-semibold hover:bg-red-600 transition-colors cursor-pointer whitespace-nowrap flex items-center justify-center gap-2">
