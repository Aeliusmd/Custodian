...
export default function AllDocumentsPage() {
  ...
  const loadDocuments = useCallback(async () => {
    try {
      ...
      const [activeRes, archivedRes] = await Promise.all([
        fetch(`${API_BASE_URL}/protected/user/${userId}/documents`, { credentials: 'include' }),
        fetch(`${API_BASE_URL}/protected/user/${userId}/documents?archived=1`, { credentials: 'include' }),
      ]);
      if (!activeRes.ok) {
        const body = await activeRes.json().catch(() => ({})) as { message?: string };
        throw new Error(body.message ?? `Failed to load documents (${activeRes.status})`);
      }
      const docs = await activeRes.json() as DocumentRecord[];
      setDocuments(docs);
      setCategories([...new Set(docs.map((d) => d.category))]);
      setUploaders([...new Set(docs.map((d) => d.uploadedBy))]);
      if (archivedRes.ok) {
        const archived = await archivedRes.json() as DocumentRecord[];
  ...
  const handleArchiveSingle = async (doc: DocumentRecord) => {
    try {
      const res = await fetch(`${API_BASE_URL}/protected/user/${userId}/documents/${doc.id}/archive`, {
        method: 'PATCH',
        credentials: 'include',
  ...
  const handleBulkArchive = async () => {
    ...
    try {
      ...
      setArchivedCount((n) => n + count);
      setSelected(new Set());
      showToast(`${count} document${count > 1 ? 's' : ''} moved to archive`);
    } catch {
      showToast('Failed to archive some documents', 'error');
    }
  };

  const handleMetaSave = async (doc: DocumentRecord, metadata: Record<string, string>) => {
    try {
      const res = await fetch(`${API_BASE_URL}/protected/user/${userId}/documents/${doc.id}/metadata`, {
        method: 'PATCH',
        credentials: 'include',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (ids: string[]) => {
    try {
      await Promise.all(ids.map((id) =>
        fetch(`${API_BASE_URL}/protected/user/${userId}/documents/${id}`, {
          method: 'DELETE',
          credentials: 'include',
        })
      ));
    ...
    <div className="px-4 py-5 sm:p-6 min-h-full font-inter" onClick={() => setActiveMenu(null)}>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between mb-6">
        <div className="min-w-0">
          ...
          <p className="text-sm text-gray-400 mt-0.5">{loading ? 'Loading...' : `${documents.length} total documents`}</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <Link
            href={`/user/${userId}/documents/archived`}
