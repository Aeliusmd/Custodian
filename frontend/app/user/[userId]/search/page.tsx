...
export default function GlobalSearchPage() {
  ...
  const doSearch = useCallback(async (q: string) => {
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE_URL}/protected/user/${userId}/search?q=${encodeURIComponent(q.trim())}`, {
        credentials: 'include',
      });
      if (!res.ok) {
    ...
    <div className="px-4 py-5 sm:p-6 min-h-full font-inter">
      ...
      <div className="max-w-3xl mx-auto mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 bg-white border-2 rounded-2xl px-4 sm:px-5 py-4 transition-all focus-within:border-[#0097B2] border-gray-200">
          ...
          <button onClick={() => void doSearch(query)} className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-white text-sm font-semibold transition-colors cursor-pointer whitespace-nowrap" style={{ background: TEAL }}>
            Search
          </button>
        </div>
      </div>

      {error && <div className="max-w-3xl mx-auto mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">{error}</div>}
      ...
      {!isLoading && results.length > 0 && (
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {results.map((result) => (
              <div key={result.id} onClick={() => setSelectedDoc(result)} className="bg-white rounded-2xl border border-gray-200 p-5 hover:border-[#0097B2]/40 transition-all cursor-pointer group">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 mb-1.5">
                      <h3 className="text-sm font-bold text-[#1a2340] group-hover:text-[#0097B2] transition-colors">{highlightText(result.documentName, submitted)}</h3>
                      <i className="ri-external-link-line text-gray-300 group-hover:text-[#0097B2] flex-shrink-0 transition-colors" />
                    </div>
                    <div className="flex items-center gap-3 mb-3">
                      <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${CATEGORY_COLORS[result.category] ?? 'bg-gray-100 text-gray-600'}`}>{highlightText(result.category, submitted)}</span>
                      <span className="text-xs text-gray-400"><i className="ri-calendar-line mr-1" />{result.uploadDate}</span>
                      <span className="text-xs text-gray-400"><i className="ri-user-line mr-1" />{result.uploadedBy}</span>
                    </div>
          ...
          <div className="relative bg-white rounded-2xl w-full max-w-2xl mx-4 overflow-hidden max-h-[85vh] flex flex-col">
            ...
            <div className="px-6 py-4 border-t border-gray-100 flex gap-3 flex-shrink-0">
              ...
              <button className="flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-white text-sm font-medium transition-colors cursor-pointer whitespace-nowrap" style={{ background: TEAL }} type="button">
                <i className="ri-eye-line" /> Open Document
              </button>
            </div>
          </div>
        </div>
