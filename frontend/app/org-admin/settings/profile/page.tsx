...
export default function AdminProfilePage() {
  ...
  const handleProfileSave = async () => {
    ...
    if (Object.keys(e).length > 0 || !initialProfile) {
      return;
    }
  ...
  if (isLoading) {
    return (
      <div className="p-4 sm:p-6 lg:p-8 font-inter min-h-full">
        <div className="max-w-3xl bg-white rounded-2xl border border-gray-200 p-10 text-center text-gray-400">
          ...
          <div className="p-4 sm:p-6 space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  className={`w-full border rounded-lg px-3.5 py-2.5 text-sm text-[#1a2340] outline-none transition-all ${
                    profileErrors.firstName
                      ? 'border-red-300 bg-red-50'
                      : 'border-gray-200 focus:border-[#0097B2]'
                  }`}
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 pt-1">
              <button
                ...
                className="w-full sm:w-auto px-5 py-2.5 rounded-lg text-white text-sm font-semibold transition-colors cursor-pointer whitespace-nowrap"
              ...
              <button
                ...
                className="w-full sm:w-auto px-5 py-2.5 rounded-lg text-sm font-semibold text-gray-600 border border-gray-200 hover:bg-gray-50 transition-colors cursor-pointer whitespace-nowrap"
              >
                Cancel
              </button>
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          ...
          <div className="p-4 sm:p-6 space-y-5">
            ...
            <div>
              ...
              <div
                className={`flex items-center border rounded-lg px-3.5 py-2.5 gap-2 transition-all ${
                  passErrors.currentPass
                    ? 'border-red-300 bg-red-50'
                    : 'border-gray-200 focus-within:border-[#0097B2]'
                }`}
              >
                <input
                ...
                <button
                  ...
                  className="text-gray-400 hover:text-gray-600 cursor-pointer w-4 h-4 flex items-center justify-center"
                >
                  <i className={showCurrent ? 'ri-eye-off-line' : 'ri-eye-line'} />
                </button>
              </div>
              {passErrors.currentPass && (
                <p className="text-xs text-red-500 mt-1">{passErrors.currentPass}</p>
              )}
            </div>

            {/* New + Confirm in 2 columns */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                {passErrors.newPass && (
                  <p className="text-xs text-red-500 mt-1">{passErrors.newPass}</p>
                )}
                {passwordStrength && (
                  <div className="mt-2">
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Strength:{' '}
                      <span className="font-semibold text-[#1a2340]">{passwordStrength.label}</span>
                    </p>
                  </div>
                )}
              </div>
              <div>
                ...
                {passErrors.confirmPass && (
                  <p className="text-xs text-red-500 mt-1">{passErrors.confirmPass}</p>
                )}
              </div>
            </div>
