...
/** Types handled by the external OCR HTTP service. */
export const extensionUsesExternalOcrService = (ext: string): boolean => {
  return ext === ".pdf" || (IMAGE_EXT as readonly string[]).includes(ext);
};

/**
 * Plain text extracted in Node without the external OCR service.
 * PDF support is limited to the embedded text layer.
 */
